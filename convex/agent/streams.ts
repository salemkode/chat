...
// NOTE: These are now in convex/lib/
import type {
...
import { fromUIMessages } from "../lib/UIMessages.js";
...
export async function finishHandler(
  ...
  const cleanupFnId = await ctx.scheduler.runAfter(
    DELETE_STREAM_DELAY,
    deleteStreamsPageForThreadIdMutation,
...
export async function deleteStreamsPageForThreadId(
  ...
  const stateKinds = ["streaming", "finished", "aborted"] as const;
  ...
  handler: async (
    ...
    if (!result.isDone) {
      await ctx.scheduler.runAfter(
        0,
        deleteAllStreamsForThreadIdAsync,
        {
