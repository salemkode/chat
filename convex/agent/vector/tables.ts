...
export const VectorTableNames = VectorDimensions.map(
  (d) => `v${d}`,
) as `v${(typeof VectorDimensions)[number]}`[];
...
export function getVectorTableName(dimension: VectorDimension) {
  return `v${dimension}` as VectorTableName;
}
...
const tables: {
  [K in keyof typeof VectorDimensions &
    number as `v${(typeof VectorDimensions)[K]}`]: VectorTable<
  ...
  `v${(typeof VectorDimensions)[number]}`,
