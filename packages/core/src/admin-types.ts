...
const ICON_TYPES = ['brand', 'emoji', 'lucide', 'phosphor', 'upload'] as const
const APP_PLANS = ['free', 'pro'] as const

export const RATE_LIMIT_SCOPES = ['global', 'user'] as const
export const RATE_LIMIT_KINDS = ['fixed window', 'token bucket'] as const
...
/** Parse a raw string into an `IconType`, returning `undefined` if it isn't one. */
export function normalizeIconType(value: string | undefined): IconType | undefined {
  return value !== undefined && isIconType(value) ? value : undefined
}
...
export { APP_PLANS, ICON_TYPES, PROVIDER_TYPES }
