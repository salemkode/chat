...
export type ConvexQueryCacheOptions = {
   ...
   * query alive even after all references in the app have dropped.
   *
   * @default 300000
   */
  expiration?: number
...
type CachedQuery = {
  refs: Set<string>
  evictTimer: ReturnType<typeof globalThis.setTimeout> | null
  unsub: () => void
}
    ...
    if (queryKey) {
      ...
      if (cq?.refs.size === 0) {
        ...
        if (this.idle === this.maxIdleEntries) {
          ...
          this.idle += 1
          const evictTimer = globalThis.setTimeout(() => {
            this.idle -= 1
            remove()
          }, this.timeout)
