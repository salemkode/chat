...
function createLocalAttachment(args: {
  ...
  const mediaType = resolveAttachmentMediaType({
    ...
    defaultMediaType: args.source === 'files' ? 'application/octet-stream' : 'image/jpeg',
  })
...
function StatusBanner({ status }: { status: SheetStatus }) {
  ...
  return (
    <View className="mx-5 mt-2 flex-row items-start gap-3 rounded-2xl border border-border/70 bg-card px-4 py-3">
      <Icon icon={IconComponent} style={{ width: 16, height: 16, color: tone, marginTop: 2 }} />
      <Text className="flex-1 text-sm leading-5 text-foreground">{status.message}</Text>
    </View>
  )
}
...
export function ChatAttachmentSheet() {
  ...
  const { attachmentMediaTypes, attachmentsSupported, imageAttachmentsSupported, selectedModel } =
    useModel()
  ...
  const projectDetail = useMemo(() => {
    ...
    return (
      projects.find((project) => project.id === projectId)?.name ?? threadProject?.name ?? 'Project'
    )
  ...
  const onPickFiles = useCallback(() => {
    ...
    void runPickerSession(async () => {
      ...
      try {
        ...
        setStatus({
          kind: 'error',
          message: error instanceof Error ? error.message : 'Unable to open the file picker.',
        })
      }
    })
  }, [
  ...
  const onPickPhotos = useCallback(() => {
    ...
    void runPickerSession(async () => {
      ...
      try {
        ...
        setStatus({
          kind: 'error',
          message: error instanceof Error ? error.message : 'Unable to open the photo library.',
        })
      }
    })
  }, [commitAttachments, imageAttachmentsSupported, runPickerSession, validateAttachments])
  ...
  const onTakePhoto = useCallback(() => {
    ...
    void runPickerSession(async () => {
      ...
      try {
        ...
        if (!photo.ok) {
          setStatus({
            kind: photo.reason === 'permission-denied' ? 'permission-denied' : 'cancelled',
            message:
