...
import { COMPOSER_LIST_BOTTOM_GAP } from './composer-layout'
...
export function Conversation({
  ...
  const scrollButtonBottomShared = useSharedValue(68 + COMPOSER_LIST_BOTTOM_GAP)

  const syncComposerHeight = useCallback(
    (composerContentHeight: number) => {
      composerHeight.value = composerContentHeight
      scrollButtonBottomShared.value = composerContentHeight + COMPOSER_LIST_BOTTOM_GAP
    },
    [composerHeight, scrollButtonBottomShared],
  )
  ...
  const scrollButtonStyle = useAnimatedStyle(() => {
    ...
    const shouldShowScrollButton =
      maxMessageScrollY > SCROLL_THRESHOLD && maxMessageScrollY - scrollY.value > SCROLL_THRESHOLD
    ...
    return {
      ...
      bottom: scrollButtonBottomShared.value + 12,
    }
  })

  const onPromptInputLayout = useCallback(
    (e: LayoutChangeEvent) => {
      syncComposerHeight(e.nativeEvent.layout.height)
    },
    [syncComposerHeight],
  )
  ...
  const listContentContainerStyle = useMemo(
    () => ({
      ...
      paddingTop: 16,
      paddingBottom: COMPOSER_LIST_BOTTOM_GAP,
    }),
    [],
  )
...
export function ConversationScrollButton() {
  ...
  const scrollButton = IS_ANDROID ? (
    ...
    >
      <SymbolImage name="chevron.down" size={18} tintColorClassName="accent-muted-foreground" />
    </Pressable>
  ) : (
    <TouchableGlass
      onPress={scrollToBottom}
...
export function ConversationEmptyState({
  ...
  return (
    <View className="flex-1 justify-center items-center gap-2">
      ...
      />
      <Text className="text-xl font-semibold text-foreground dark:text-foreground">{title}</Text>
      {description && (
