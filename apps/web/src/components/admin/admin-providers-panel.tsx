...
type AdminProvidersPanelProps = Pick<
  AdminOutletContext,
  'dashboard' | 'onOpenProviderDialog'
>
              ...
              {providers.map((provider: AdminProvider) => (
                <TableRow key={provider._id}>
                  ...
                  <TableCell>
                    <div className="space-y-1 text-xs text-muted-foreground">
                      <p>
                        {formatCompactNumber(provider.usage.requests)} requests
                      </p>
                      <p>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          void discovery.inspectSavedProvider(provider)
                        }
        ...
        <Card className="border-border bg-card shadow-[0_18px_50px_rgba(15,23,42,0.08)] dark:shadow-[0_18px_50px_rgba(0,0,0,0.35)]">
          ...
          <CardContent className="grid gap-4">
            <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
              <Badge
                variant={
                  discovery.discoveryResult.ok ? 'default' : 'destructive'
                }
              >
                {discovery.discoveryResult.ok
                  ? 'Discovery succeeded'
                ...
                <div className="flex flex-col gap-3 rounded-xl border border-border bg-muted/20 p-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Inspect keeps the full provider catalog available. Search
                      by name, model ID, owner, or capabilities, then import only
                      the models you need.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button
                      ...
                      disabled={
                        discovery.selectedDiscoveryModelIds.length === 0
                      }
                    >
                      Clear
                    </Button>
                      ...
                      >
                        {discovery.discoveryResult.models.map(
                          (
                            ...
                            const isImported =
                              discovery.existingDiscoveredModelIds.has(
                                model.modelId,
                              )
