                ...
                <div className="grid gap-4 rounded-2xl bg-muted/20 p-5">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        selectedModelProvider ? void actions.onInspectSelectedProvider() : undefined
                      }
                      disabled={
                        !selectedModelProvider ||
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2">
                                <span className="font-medium">Blank custom model</span>
                                {!modelForm.modelId ? (
                                  <Badge variant="secondary">Selected</Badge>
                                ) : null}
                              </div>
                              <p className="text-xs text-muted-foreground">
                                Start from an empty form and configure the model manually.
                              </p>
                            </div>
                          </CommandItem>
                        </CommandGroup>
                    ...
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setModelForm((current) => ({
                            ...current,
                            defaultReasoningLevel: value as 'off' | 'low' | 'medium' | 'high',
                          }))
                        }
...
export function useAdminModelDialog({
  ...
  const setEditingModel = (model: Doc<'models'> | null) => updateDialog({ editingModel: model })
  ...
  const hasDiscoveryForSelectedProvider =
    discovery.discoveryResult?.ok && discovery.activeDiscoveryProviderId === modelForm.providerId
  ...
  const discoveredModelsForSelectedProvider = useMemo(() => {
    ...
    return discovery.discoveryResult.models.filter(
      (model: ProviderCatalogResult['models'][number]) => !existingModelIds.has(model.modelId),
    )
  }, [discovery.discoveryResult, hasDiscoveryForSelectedProvider, modelForm.providerId, models])

  const discoveredModelCountForSelectedProvider = hasDiscoveryForSelectedProvider
      ...
      if (model) {
        ...
        setModelForm({
          ...
          defaultReasoningLevel:
            (model.defaultReasoningLevel as 'off' | 'low' | 'medium' | 'high' | undefined) ?? 'off',
  ...
  const applyDiscoveredModelToForm = useCallback(
    (discoveredModel: ProviderCatalogResult['models'][number]) => {
      ...
      setModelForm((current) => ({
        ...
        contextWindow: discoveredModel.contextWindow ? String(discoveredModel.contextWindow) : '',
  ...
  const handleSaveModel = useCallback(() => {
    ...
    const payload = {
      ...
      icon: modelForm.iconType === 'upload' ? undefined : modelForm.icon?.trim() || undefined,
      iconType: modelForm.iconType,
      iconId:
      ...
      maxOutputTokens: modelForm.maxOutputTokens ? Number(modelForm.maxOutputTokens) : undefined,
  ...
  }, [addModel, defaultProviderId, editingModel, modelForm, nextModelSortOrder, updateModel])
