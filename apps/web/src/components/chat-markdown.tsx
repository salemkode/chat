...
function extractFenceLanguage(className: string | undefined): { language: string; label: string } {
...
function MarkdownCodeBlock({
  ...
  /** Merged onto the inner content wrapper (e.g. overflow for diagrams). */
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'overflow-x-auto pt-10 pb-4 [&_.shiki]:bg-transparent! [&_pre]:m-0! [&_pre]:bg-transparent! [&_pre]:px-4! [&_pre]:py-0! [&_pre]:text-left [&_pre]:text-[13px]',
          contentClassName,
        )}
      >
        {children}
...
export const ChatMarkdown = React.memo(function ChatMarkdown({
  ...
  const normalizedText = useMemo(() => normalizeAccidentalIndentedFilenameLabels(text), [text])
  ...
  const markdownComponents = useMemo<Components>(
    () => ({
      ...
      pre({ children, ...props }) {
        ...
        if (fenceLanguage.language === 'mermaid') {
          return (
            <MarkdownCodeBlock
              code={codeBlock.code}
            ...
            >
              {isStreaming ? (
                <div className="overflow-x-auto px-4 pt-10 pb-4">
                  <pre className="m-0 bg-transparent p-0 text-left font-mono text-[13px] whitespace-pre-wrap text-foreground/90">
                    {codeBlock.code}
                  </pre>
                </div>
              ) : (
                <ChatMermaidViewer chart={codeBlock.code} resolvedTheme={resolvedTheme} />
              )}
            </MarkdownCodeBlock>
          )
        }
        ...
        return (
          <MarkdownCodeBlock code={codeBlock.code} languageLabel={fenceLanguage.label}>
    ...
    [isStreaming, resolvedTheme, shikiTheme],
  )
