...
export function MessageActivityTimeline({ parts, messageStatus }: MessageActivityTimelineProps) {
  const steps = useMemo(() => buildActivitySteps(parts, messageStatus), [messageStatus, parts])
...
function ActivityStepRow({
  ...
  if (step.kind === 'reasoning') {
    return <ReasoningRow step={step} isLast={isLast} showActiveLoading={showActiveLoading} />
  }

  if (isSearchTool(step.toolName)) {
    return <SearchToolRow step={step} isLast={isLast} showActiveLoading={showActiveLoading} />
  }
  ...
  return (
    <ChainOfThoughtStep open={false} className={cn(isLast && 'pb-0')} data-last={isLast}>
      <ChainOfThoughtItem
        className={cn(
