...
- **Mobile** (`apps/mobile/src/components/chat/attachment-sheet.tsx`, `apps/mobile/src/components/chat/attachment-context.tsx`, `apps/mobile/src/components/share-intent-bridge.tsx`, `apps/mobile/src/hooks/use-send-message.ts`): the attachment sheet adds files from the system document picker, photo library, or camera into shared composer state. The Expo Sharing bridge also accepts text, URLs, images, and PDFs shared from other apps, clears the active thread, and preloads a new composer with the shared text and/or attachments. The selected model’s resolved attachment media types gate which picks are accepted. On send, the app reads each local URI as raw bytes (via `expo-file-system` `File` when available, otherwise `fetch(uri).blob()`), `POST`s that body to `agents.generateAttachmentUploadUrl` with a `Content-Type` matching the resolved MIME type (same contract as web — not multipart form data), and forwards `{ storageId, filename, mediaType }` to `agents.generateMessage`. Filenames are normalized (NFC, no path segments) before upload metadata is sent.
